...
typedef enum GX_FEATURE_ID
{
    ...
    GX_ENUM_REGION_SEND_MODE              = 1019 | GX_FEATURE_ENUM | GX_FEATURE_LEVEL_REMOTE_DEV,    ///< ROI输出模式， 参考GX_REGION_SEND_MODE_ENTRY
    GX_ENUM_REGION_MODE                   = 1020 | GX_FEATURE_ENUM | GX_FEATURE_LEVEL_REMOTE_DEV,    ///< 区域开关， 参考GX_REGION_MODE_ENTRY
    GX_ENUM_RREGION_SELECTOR              = 1021 | GX_FEATURE_ENUM | GX_FEATURE_LEVEL_REMOTE_DEV,    ///< 区域选择，参考GX_REGION_SELECTOR_ENTRY
...
{
    GX_REGION_SEND_SINGLE_ROI_MODE       = 0,   ///< 单ROI模式
    GX_REGION_SEND_MULTI_ROI_MODE        = 1    ///< 多ROI模式
}GX_REGION_SEND_MODE;
...
typedef struct GX_FRAME_CALLBACK_PARAM
{
    ...
    int32_t             nOffsetX;           ///< 图像X方向偏移
    int32_t             nOffsetY;           ///< 图像Y方向偏移
...
\return    GX_STATUS_SUCCESS             操作成功，没有发生错误
           ...
           GX_STATUS_INVALID_HANDLE      用户传入非法的句柄
           GX_STATUS_INVALID_ACCESS      设备访问模式错误
           GX_STATUS_INVALID_CALL        无效的接口调用
           GX_STATUS_ERROR               不期望发生的未明确指明的内部错误
           其它错误情况请参见GX_STATUS_LIST 
*/
//----------------------------------------------------------------------------------
...
\param [in]  hDevice        设备句柄
\param [out] ppFrameBuffer  图像数据，如果函数返回失败，图像数据是无效的
...
//----------------------------------------------------------------------------------
/**
\brief      获取队列中所有图像，在图像处理完成后，需要调用GXQAllBufs接口，否则采集将无法继续
\param [in]     hDevice                 设备句柄
\param [out]    ppFrameBufferArray      图像数据指针的数组
\param [in]     nFrameBufferArraySize   图像数组申请个数
\param [out]        pnFrameCount                返回图像实际填充图像个数
\param [in]     nTimeOut                超时时间(单位ms)
\return    GX_STATUS_SUCCESS            操作成功，没有发生错误
           GX_STATUS_NOT_INIT_API       没有调用GXInitLib初始化库
           GX_STATUS_INVALID_HANDLE     用户传入非法的句柄
...
//----------------------------------------------------------------------------------
/**
\brief      将队列中所有Buf放回GxIAPI库，继续用于采集
\param [in]  hDevice        设备句柄
\return    GX_STATUS_SUCCESS             操作成功，没有发生错误
           GX_STATUS_NOT_INIT_API        没有调用GXInitLib初始化库
           GX_STATUS_INVALID_HANDLE      用户传入非法的句柄
