...
void CGxViewer::on_actionAbout_triggered()
{
    ...
    QMessageBox::about(this, tr("About"), tr("Copyright (C)  2015-2018\n\nLibVer. : %1\nDemoVer. : %2")
                                            .arg(szLibVersion)
                                            .arg(tr("v1.0.1807.9271")));
...
void CGxViewer::GetDeviceInitParam()
{
    ...
    if (bColorFilter)
    {
        ...
        if (!bRet)
        {
            QMessageBox::about(NULL, "Error", "Open Device Failed : Allocate LUT memory failed, image improvement disabled");
